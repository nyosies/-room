...
<head>
    ...
    if (isset($_POST["submit"]) && $_POST["submit"]) {
        if( $_POST["submit"]=="发布") {
            $sqlAddNews = "INSERT INTO `tbl_news` VALUES (NULL , '" . $_SESSION["admin_ID"] . "', '" . $_POST['news_title'] . "', '" . $_POST["news_content"] . "', '" . date('y-m-d') . "')";
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <div class="alert alert-info">
                    ...
                    <strong>欢迎来到:</strong>学生管理系统
                </div>
            </div>
        </div>
        <div class="btn-toolbar">
            <button class="btn btn-primary"><a href="#publish" role="button" data-toggle="modal"><font
                            color="#F7F8F7"><i class="icon-plus"></i>发布信息</font></a></button>
        </div>
                ...
                <table width="996">
                    ...
                    if ($resN = mysqli_query($db, $sqlNews)) {
                        while ($rowsN = mysqli_fetch_assoc($resN)) {
                            ...
                            ?>
                            <td>
                                <form action="1_DRM_index.php" method="post">
                                    <input type="hidden" name="news_ID" value="<?php echo $rowsN["news_ID"]; ?>">
                                    <input type="hidden" name="type" value="new">
                                    <input type="submit" class="btn btn-danger" onclick="return confirm('确定要删除吗?');"
