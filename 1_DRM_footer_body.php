<div class="navbar">
        <div class="navbar-inner">
                <ul class="nav pull-right">
                    
                    <li><a href="http://www.sylu.edu.cn/sylusite/" class="hidden-phone visible-tablet visible-desktop" target="_blank" role="button">账户</a></li>
                    <li><a href="1_DRM_index.php" class="hidden-phone visible-tablet visible-desktop" role="button"> 管理员</a></li>
                    
                </ul>
                <a class="brand" href="#"><span class="first"></span> <span class="second">管理员端</span></a>
        </div>
</div>
<div class="sidebar-nav">
     <ul class="nav nav-list collapse in">
         <li><a href="1_DRM_index.php">首页</a></li>
         <li><a href="1_DRM_dor_list.php">寝室管理</a></li>
         <li><a href="1_DRM_stu_list.php">学生管理</a></li>
         <li><a href="1_DRM_turn_list.php">住宿管理</a></li>
         <li><a href="reset-password.php?fix=admin">重置密码</a></li>
         <li><a href="logout.php">退出</a></li>
</div>